...
BOOST_FIXTURE_TEST_SUITE(Date, TimeFixture)
	...
	{
		BOOST_REQUIRE(CheckTime(--CTime(0, 0, 0), 23, 59, 59));
	...
	BOOST_AUTO_TEST_CASE(can_handle_assignment_addition_operator_time_overflow)
	{
		CTime time1(15, 0, 0);
		CTime time2(15, 0, 0);
		time2 += time1;

		BOOST_REQUIRE(CheckTime(time2, 6, 0, 0));
	}
	...
	BOOST_AUTO_TEST_CASE(can_handle_assignment_subtraction_overflow_day_time)
	{
		CTime time1(20);
		CTime time2(40);
		time1 -= time2;

		BOOST_REQUIRE(CheckTime(time1, 23, 59, 40));
	}
	...
	BOOST_AUTO_TEST_CASE(can_use_assignment_multiplocation_operator)
	{
		...
		BOOST_REQUIRE(CheckTime(time1, 0, 0, 50));
	}

	BOOST_AUTO_TEST_CASE(can_handle_multiplocation_operator_overflow_result)
	{
		CTime time1(1, 0, 0);
		time1 *= 48;
		...
		BOOST_REQUIRE(CheckTime(time2, 0, 30, 0));
	}
	...
	BOOST_AUTO_TEST_CASE(can_use_assignment_division_operator_and_get_number)
	{
		CTime time3(10);
		CTime time4(100);
		BOOST_CHECK_EQUAL(time3 /= time4, 0);

		CTime time5(300);
		CTime time6(100);
		BOOST_CHECK_EQUAL(time5 /= time6, 3);
	...
	BOOST_AUTO_TEST_CASE(can_handle_division_by_zero)
	{
		...
		BOOST_REQUIRE_THROW(CTime(10) / CTime(0), std::invalid_argument);
		BOOST_REQUIRE_THROW(CTime(10) / CTime(-10), std::invalid_argument);
	}
	...
	BOOST_AUTO_TEST_CASE(can_use_comparison_operator)
	{
		...
		BOOST_CHECK_EQUAL((time1 == time2), true);
		BOOST_CHECK_EQUAL((time1 == time3), false);

		BOOST_CHECK_EQUAL((time1 != time3), true);
		BOOST_CHECK_EQUAL((time1 != time1), false);
	}

	BOOST_AUTO_TEST_CASE(can_use_strict_comparison_operator_more)
	{
		CTime time1(10);
		CTime time2(20);
	...
	BOOST_AUTO_TEST_CASE(can_use_strict_comparison_operator_less)
	{
		CTime time1(10);
		CTime time2(20);		
	...
	BOOST_AUTO_TEST_CASE(can_use_nonstrict_comparison_operator_more)
	{
		CTime time1(40);
		CTime time2(50);
	...
	BOOST_AUTO_TEST_CASE(can_use_nonstrict_comparison_operator_less)
	{
		CTime time1(40);
		CTime time2(50);
	...
	BOOST_AUTO_TEST_CASE(this_test_will_not_pass)
	{
		CTime t(23, 0, 0);
		t *= 3;
		BOOST_CHECK(t == CTime(21, 0, 0));
	}
BOOST_AUTO_TEST_SUITE_END()

void DoSomething(const CTime& t1, const CTime& t2)
{
	CTime t = t1 - t2;
}